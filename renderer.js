...
function renderScreenDynamics(ctx, frame, lastStaticRedraw, halfWidth, doubleWidth, metrics, clearAll) {
  ...
  function renderRow(y) {
    ...
    for (var x  = 0; x < receiver.columns; x++) {
      ...
      if (cursor) {
        ctx.clearRect(x*halfWidth, Math.round(y*metrics.height), halfWidth*width, Math.round(metrics.height));
        ...
        if (window_focused) {
          ...
          ctx.fillRect(x * halfWidth, Math.round(y * metrics.height),
                       halfWidth * width, Math.round(metrics.height));
        } else {
...
function setup()
{
  ...
  var evaluator = new Evaluator(opts);
  ...
  term.write = function (str) {
    for (var c of str) {
      if (c == "\r") {
        receiver.feed("\r\n");
        // evaluator.feed(line_buffer + "\n");
        try {
          var exp = JSON.parse(line_buffer)
          evaluator.startEval(exp, []);
          setTimeout(function loop() {
            if (!evaluator.evaluating) {
              try {
                receiver.feed(JSON.stringify(evaluator.value) + "\r\n");
              } catch (e) {
                receiver.feed(e.message + "\r\n");
              }
              force_redraw = true;
            } else {
              var t0 = +new Date();
        ...
        var sym = String.fromCodePoint(c.codePointAt(0) + 64);
        line_buffer += c;
        receiver.feed("^" + sym); force_redraw = true;
      } else if (c == "\x7f") { // DEL
        if (line_buffer.length > 0) {
          if (isctrl(line_buffer[line_buffer.length - 1])) {
            receiver.feed("\b\b  \b\b");
          } else {
            receiver.feed("\b \b"); // 全角だとまずいね。
          }
